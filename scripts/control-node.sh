...
containers_per_node=6  # 每个服务器上的容器数量，相当于是多少层
...
for j in $(seq 1 $containers_per_node); do
    for i in $(seq 1 $NODE_NUM); do
        ...
        ssh "$ssh_user_host" -- "cd ~/htadkg && docker-compose run -p $external_port:$external_port adkg python3 -m scripts.admpc_dynamic_run -d -f conf/admpc_cloud_16_6/local.$file_num.json -time 12" &
