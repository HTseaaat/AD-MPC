...
# 先调用 docker run 开启docker 容器
for ID in $(seq 1 $NUM_NODES); do
    ssh_user_host="${NODE_SSH_USERNAME}@${NODE_IPS[$ID - 1]}"
    ssh "$ssh_user_host" -- "docker-compose run adkg bash"
...
for ID in $(seq 1 $NUM_NODES); do
    ssh_user_host="${NODE_SSH_USERNAME}@${NODE_IPS[$ID - 1]}"
    ssh "$ssh_user_host" -- "${CMD} -d -f ${CONFIG_PATH}.${ID}.json"
    # ssh "$ssh_user_host" -- "docker load -i ~/sdumoe-docker/sdumoe-chain-backend.docker.image.tar.xz"
done
